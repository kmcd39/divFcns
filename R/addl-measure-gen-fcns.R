...
#'
#' Projects a shapefile to an Azimuth Equidisant Projection centered on a
#' supplied centroid, or on a centroid generated for the supplied shapefile.
#' This allows distances to be calculated straightforwardly using meters distance unit.
#' @export
local_equidistant_project <- function(st_df, projection_center = NULL) {
  st_df <- st_sf(st_df)
  if (is.null(projection_center))
    projection_center <- geosphere::centroid(st_coordinates(st_df)[,c("X", "Y")])
...
get_azimuthal_projection_centroid <- function(st_df) {
  # longlat project input and extract coordinates
  st_df <-
    st_coordinates(
      st_transform(st_as_sf(st_df),
                   4326))
  # if geosphere can triangulate coordinates into polygon, calculate centroid
  if (nrow(st_df) > 2)
    projection_centroid <-
